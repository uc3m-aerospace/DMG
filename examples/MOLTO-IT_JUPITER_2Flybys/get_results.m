...
%   
Time = 2.1 + i*0.1;
load(['VE_',num2str(Time),'.mat'])
%
fb_b =[setup.flyby_planet,setup.arrival_planet];
%
subpath = ['./','VE_',num2str(Time)];
%
if not ( isdir(subpath) )
   mkdir(subpath)
end
%
x = [ solution(1).state(:,1),solution(2).state(:,1),solution(3).state(:,1)];
y = [ solution(1).state(:,2),solution(2).state(:,2),solution(3).state(:,2)];
z = [ solution(1).state(:,3),solution(2).state(:,3),solution(3).state(:,3)];
%
fig = figure;
plot3(x,y,z)
%

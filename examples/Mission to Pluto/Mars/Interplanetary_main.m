...
for iphase = 1:n_phases
    ...
    if iphase == 1
        limits(iphase).state.min(7,:)   = [1 1e-1 1e-1];
    else
        limits(iphase).state.min(7,:)   = [1e-1 1e-1 1e-1];
    end
    ...
    %
    if iphase == 1
        limits(iphase).parameter.min    = [setup.hpmin(iphase)/1e6;-pi;-pi;-pi;Isp(1);vinf_min ;0];
        limits(iphase).parameter.max    = [setup.hpmax(iphase)/1e6;pi; pi; pi;Isp(2);vinf_max  ;1];
    elseif iphase < n_phases
        limits(iphase).parameter.min    = [setup.hpmin(iphase)/1e6;-pi;Isp(1)/1000;vinf_min  ;0];
        limits(iphase).parameter.max    = [setup.hpmax(iphase)/1e6;pi;Isp(2)/1000;vinf_max  ; 1];
    else
        limits(iphase).parameter.min    = [Isp(1)/1000;vinf_min ;0];
        limits(iphase).parameter.max    = [Isp(2)/1000;vinf_max ;1];
    end
    %
    % CONSTRAINTS LIMITS
    %
    ...
    m_guess     = m_0*exp(-mean(T_guess*0.00001)/(setup.g0/setup.ac*setup.Isp/setup.tc).*(t_guess-t_guess(1)));
    m_guess     = linspace(0.9,0.9,numel(t_guess));
    %
    ...
    guess(iphase).control(:,1) = ones(size(T_guess)).*(T_guess>0.2);
    guess(iphase).control(:,2) = alpha_guess;
    guess(iphase).control(:,3) = 0;
    guess(iphase).control(:,4) = Ispguess;
    %
    %plot(t_guess,m_guess)
    %hold on
    if iphase == 1
        %
        guess(iphase).parameter    = [hpguess/1e6;cguess;0;0;Ispguess/1000;vinf0_guess;initial_date];
        %
    elseif iphase < n_phases
        %
        guess(iphase).parameter    = [hpguess/1e6;cguess;Ispguess/1000;vinf0_guess;initial_date];
        %
    else
        %
        guess(iphase).parameter    = [Ispguess/1000;vinf0_guess;initial_date];
        %
    end
    %
    % ----------------------------------------------------------------------------------------
...
for j = 1:n_fb + 2
    ...
    T   = 2*pi*sqrt(norm(rp)^3)*setup.tc;

...
setup.ratio_mass = 0.9;             % Ratio between final mass and initial mass
setup.time =222 *24*3600;           % [s]Corresponds with the time for a Hohmann transfer
...
rx_f = mission(end,2)/setup.lc;
ry_f = mission(end,3)/setup.lc;
vx_f = mission(end,5)/setup.vc;
vy_f = mission(end,6)/setup.vc;
...
%% Maximum conditions
rx_max = 1.1*r_callisto_ad;
ry_max = 1.1*r_callisto_ad;
vx_max = 1.5*v_ganymede_ad;
vy_max = 1.5*v_ganymede_ad;
m_max = m_0;
...
limits(iphase).nodes = 400; % Setting the number of nodes
...
guess(iphase).state(:,1)   = mission(:,2)*1/lc;
guess(iphase).state(:,2)   = mission(:,3)*1/lc;
guess(iphase).state(:,3)   = mission(:,5)*1/vc;
guess(iphase).state(:,4)   = mission(:,6)*1/vc;
guess(iphase).state(:,5)   = mission(:,21)/1000;
guess(iphase).control(:,1) = ones(size(mission(:,2)));
guess(iphase).control(:,2) = atan2(mission(:,23),mission(:,22));
